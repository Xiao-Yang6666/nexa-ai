...
```
.
├── backend/    # Maven reactor: nexa-common 公共组件 + nexa-service Spring Boot 服务
└── frontend/   # Next.js 14 + React + TypeScript + TanStack Query
```

## 后端

技术栈：Spring Boot 3.2、Java 21（虚拟线程）、JPA/Hibernate 6、PostgreSQL、Flyway。后端已拆为两个 Maven 模块：`nexa-common` 提供共享内核、统一响应、全局异常处理和安全公共构件；`nexa-service` 是业务 Spring Boot 服务并通过 Maven 依赖引用公共组件。

```bash
cd backend
cp .env.example .env                     # 填入真实数据库/密钥
# export 环境变量后启动（maven）
mvn -pl nexa-service -am spring-boot:run # 默认 :8080，Flyway 自动建表
mvn test                                 # reactor 全量单元测试
```
