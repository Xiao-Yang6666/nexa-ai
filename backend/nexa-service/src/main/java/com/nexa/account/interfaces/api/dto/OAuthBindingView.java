...
/**
 * 用户 OAuth 绑定视图 DTO（管理端查询 F-1027，对齐 openapi {@code OAuthBindingView}）。
 *
 ...
 */
public record OAuthBindingView(
        Long id,
        Long providerId,
     ...
     * @return 管理端视图
     */
    public static OAuthBindingView from(OAuthBinding binding) {
        Instant createdAt = binding.createdAt();
        return new OAuthBindingView(
                binding.id(),
