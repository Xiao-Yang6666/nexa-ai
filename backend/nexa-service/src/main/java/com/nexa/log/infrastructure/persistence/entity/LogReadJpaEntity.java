package com.nexa.log.infrastructure.persistence.entity;
 ...
 *
 * <p><b>为何独立于 relay 的 {@code LogJpaEntity}</b>：DDD 上下文解耦——relay BC 负责写消费/错误日志，
 * 本（log）BC 负责读查询/统计/审计/清理，两个 bounded context 各持自己的持久化实体，不跨 BC 复用
 * （避免 log BC 反向依赖 relay infra）。两实体映射同一张物理表是 JPA 合法用法。</p>
 *
 * <p><b>索引声明归属</b>：物理索引由 V11 Flyway 脚本 + relay {@code LogJpaEntity} 的 {@code @Table(indexes=)}
