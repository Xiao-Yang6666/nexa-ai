...
/**
 * 令牌客户视图 DTO（接口层出参，对齐 openapi TokenUserView，F-3002/F-3003/F-3001/F-3006）。
 *
...
public record TokenUserView(
     ...
     * @return 客户视图 DTO
     */
    public static TokenUserView from(Token token) {
        return new TokenUserView(
                token.id(),
                token.name(),
                token.maskedKey(),
                token.status().code(),
