 ...
 */
public record HardwareTypesView(
     ...
     * @return 出参视图
     */
    public static HardwareTypesView from(HardwareCatalog catalog) {
        List<Map<String, Object>> items = catalog.hardwareTypes().stream()
                .map(HardwareType::attributes)
                .toList();
        return new HardwareTypesView(items, catalog.total(), catalog.totalAvailable());
    }
}
