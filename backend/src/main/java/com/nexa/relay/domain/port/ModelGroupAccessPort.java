 ...
 * {@code com.nexa.modelgroup.infrastructure.access.ModelGroupAccessAdapter}（依赖倒置）。</p>
 *
 * <p>判定语义（{@link #isAccessible}）：
 * <ul>
 *   <li>分组 code 无对应存活模型组 → 放行（{@code true}，回落旧行为，分组不受模型组管控）；</li>
 *   <li>命中模型组为 PUBLIC / AUTO_LEVEL → 放行（公开人人可用；自动按等级，闸门不在此收窄）；</li>
 *   <li>命中模型组为 PRIVATE → 仅当 token 级或 user 级有显式授权才放行，否则拒绝。</li>
 * </ul>
...
public interface ModelGroupAccessPort {
     ...
     * @return 有权使用返回 {@code true}；私有组且无授权返回 {@code false}
     */
    boolean isAccessible(String groupCode, long userId, Long tokenId);
}
