...
/**
 * 自定义 OAuth provider 视图 DTO（对齐 openapi {@code CustomOAuthProviderView}，RootAuth）。
 *
 ...
 */
public record CustomOAuthProviderView(Long id,
                                      String name,
                                      String clientId,
     ...
     */
    public static CustomOAuthProviderView from(CustomOAuthProvider p) {
        List<String> scopeList = p.scopes() == null || p.scopes().isBlank()
                ? List.of()
                : Arrays.stream(p.scopes().trim().split("\\s+")).toList();
        return new CustomOAuthProviderView(
                p.id(),
                p.name(),
                p.clientId(),
