package com.nexa.application.routing.port;
 ...
 * {@link com.nexa.application.routing.ResolveChannelRouteUseCase} 在 auto 分组跨组重试调度（CH-5）中，
 * 通过本端口委托实际的渠道查询与加权随机抽签——把「选哪个组、该不该切」的调度决策（CH-5）与
 * 「在一个组里怎么选一个渠道」的查询/抽签（CH-2）解耦：</p>
 *
 * <ul>
 *   <li>解耦后 CH-5 调度逻辑（{@link com.nexa.domain.routing.service.CrossGroupRetryScheduler}）零框架依赖、可纯单测。</li>
