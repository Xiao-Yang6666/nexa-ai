package com.nexa.deployment.application.port;
 ...
 * {@link com.nexa.deployment.domain.exception.IonetIntegrationException}（接口层映射 502）。</p>
 */
public interface IonetClient {
     ...
     * {@link com.nexa.deployment.domain.exception.IonetIntegrationException}（契约「container details not found」）。</p>
     *
     * @param deploymentId 部署 ID（已校验非空）
     ...
     * {@link com.nexa.deployment.domain.exception.IonetIntegrationException}（契约「api_key is required」）；
     ...
     * {@link com.nexa.deployment.domain.exception.DeploymentNameUnavailableException}（→409）；
     * 预检/上游失败抛 {@link com.nexa.deployment.domain.exception.IonetIntegrationException}
