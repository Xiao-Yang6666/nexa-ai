...
public class ChannelRepositoryImpl implements ChannelRepository {
     ...
     */
    public ChannelRepositoryImpl(SpringDataChannelJpaRepository jpa, ObjectMapper objectMapper) {
        this.jpa = jpa;
    ...
    /**
     * JPA 实体 → 领域聚合（重建方向，走 {@link Channel#rehydrate}）。
     *
     * @param e JPA 实体
     * @return 重建的领域聚合
     */
    private Channel toDomain(ChannelJpaEntity e) {
        return Channel.rehydrate(
                e.getId(), e.getType(), e.getKey(), e.getStatus(), e.getName(), e.getWeight(),
                e.getBaseUrl(), e.getModels(), e.getGroup(), e.getPriority(), e.getAutoBan(),
                e.getBalance(), e.getUsedQuota(), e.getResponseTime(), e.getTestTime(),
                e.getModelMapping(), e.getStatusCodeMapping(), e.getTag(), e.getSetting(),
                deserializeChannelInfo(e.getChannelInfo()), e.getCreatedTime());
    }
