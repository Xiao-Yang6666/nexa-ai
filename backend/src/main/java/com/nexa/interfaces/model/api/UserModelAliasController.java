...
import com.nexa.interfaces.model.api.dto.*;
...
public class UserModelAliasController {
     ...
     * @return 成功信封，data = items[]（UserView，target 仅 A）
     */
    @GetMapping
    public ApiResponse<List<UserModelAliasUserView>> list(@CurrentActor AuthenticatedActor actor) {
        List<UserModelAliasUserView> items = useCase.listVisible(actor.userId()).stream()
                .map(UserModelAliasUserView::from).toList();
     ...
     * @return 成功信封，data = 创建后映射（UserView）
     */
    @PostMapping
    public ApiResponse<UserModelAliasUserView> create(
        ...
        return ApiResponse.okData(UserModelAliasUserView.from(created));
    }
     ...
     * @return 成功信封，data = 更新后映射（UserView）
     */
    @PutMapping("/{id}")
    public ApiResponse<UserModelAliasUserView> update(
            @CurrentActor AuthenticatedActor actor,
        ...
        return ApiResponse.okData(UserModelAliasUserView.from(updated));
    }
     ...
     *
     * <p>来源 = PublicModel Enabled=true 全集（UserView，绝不含任何 B——B 不可见三道闸之候选层闸）。
