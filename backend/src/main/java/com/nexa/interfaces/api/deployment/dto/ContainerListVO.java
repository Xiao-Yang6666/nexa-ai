...
import com.nexa.domain.deployment.model.ContainerEvent;
import com.nexa.domain.deployment.model.ContainerList;
import com.nexa.domain.deployment.model.ContainerSummary;
 ...
 * @param total      容器总数
 */
public record ContainerListVO(List<ContainerItem> containers, int total) {
     ...
     * @return 出参视图
     */
    public static ContainerListVO from(ContainerList list) {
        List<ContainerItem> items = list.containers().stream()
                .map(ContainerListVO::toItem)
                .toList();
        return new ContainerListVO(items, list.total());
    }

    private static ContainerItem toItem(ContainerSummary c) {
        List<EventItem> events = c.events().stream()
                .map(ContainerListVO::toEvent)
                .toList();
