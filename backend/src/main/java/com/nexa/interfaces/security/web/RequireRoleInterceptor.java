 ...
 * 取当前认证主体（经应用层端口 {@link CurrentActorProvider#require()}，缺失即 401），再
 ...
 *
 * <p>设计依据：backend-engineer §3.2 错误用明确类型集中翻译；与路径级鉴权互补（见 {@link RequireRole}）。
 * 取主体经应用层端口，接口层不直接依赖基础设施安全上下文持有器。</p>
 */
@Component
public class RequireRoleInterceptor implements HandlerInterceptor {
     ...
     */
    public RequireRoleInterceptor(CurrentActorProvider currentActorProvider) {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        AuthenticatedActor actor = currentActorProvider.require();
