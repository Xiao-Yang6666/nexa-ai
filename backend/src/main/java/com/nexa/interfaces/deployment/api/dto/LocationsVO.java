 ...
 * @param total     生效地域总数
 */
public record LocationsVO(List<Map<String, Object>> locations, long total) {
     ...
     * @return 出参视图
     */
    public static LocationsVO from(LocationCatalog catalog) {
        List<Map<String, Object>> items = catalog.locations().stream()
                .map(Location::attributes)
                .toList();
        return new LocationsVO(items, catalog.total());
    }
}
