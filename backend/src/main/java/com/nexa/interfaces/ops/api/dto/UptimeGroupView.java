 ...
 * @param monitors 监控项
 */
public record UptimeGroupView(String group, List<MonitorView> monitors) {
     ...
     * @return 视图
     */
    public static UptimeGroupView from(UptimeStatusClient.UptimeGroup group) {
        List<MonitorView> monitors = group.monitors().stream().map(MonitorView::from).toList();
        return new UptimeGroupView(group.group(), monitors);
    }
