package com.nexa.interfaces.telegram.api.dto;

import com.nexa.domain.account.model.User;

/**
 * Telegram 登录用户客户视图 DTO（对齐 openapi.yaml {@code UserVO}，F-1051 登录返回的 data）。
 *
 * <p><b>客户视图零敏感泄露（产品铁律）</b>：本 DTO <b>绝不</b>包含 password / passwordHash /
 * access_token，也不含成本/利润/上游真实模型/渠道等内部字段，更不含 Bot Token。字段集与
 * {@code com.nexa.interfaces.account.api.dto.UserVO} 对齐（同一 openapi UserVO 契约）；
...
public record TelegramUserVO(
     ...
     */
    public static TelegramUserVO from(User user) {
        return new TelegramUserVO(
                user.id(),
                user.username().value(),
                user.role().code(),
                user.status().code(),
