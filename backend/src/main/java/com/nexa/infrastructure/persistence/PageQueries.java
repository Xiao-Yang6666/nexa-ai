 ...
 *
 * <p><b>迁移中间态</b>：JPA → MyBatis-Plus 滚动迁移期，{@link #of} 系列产出 Spring Data {@link Pageable}
 * 供未迁域使用；{@link #mpOf} 产出 MyBatis-Plus {@link Page}（页号本就从 1 起，无需减一）供已迁域使用。
 * 两者并存，待全部域迁完后 Spring Data 重载随 JPA 一并移除。</p>
 *
 * <p>定位：无状态纯函数工具（全 static），不持有任何依赖，不进 domain（domain 不感知分页框架）。
...
public final class PageQueries {
    ...
    /**
     * 1-based 页号 + 每页条数 → MyBatis-Plus {@link Page}（MP 页号本就从 1 起，无需减一）。
     *
     * @param page1Based 页号（从 1 起，领域约定；&lt;1 归一为 1）
     * @param pageSize   每页条数（&lt;1 归一为 1）
     * @param <T>        分页记录类型
     * @return MyBatis-Plus 分页对象
     */
    public static <T> Page<T> mpOf(int page1Based, int pageSize) {
