 ...
 */
interface SpringDataModelGroupJpaRepository extends JpaRepository<ModelGroupPO, Long> {

    /** 全量按 id 升序查（管理端列表）。 */
    List<ModelGroupPO> findAllByOrderByIdAsc();

    /** 按 code 查存活模型组（中继选组）。 */
    Optional<ModelGroupPO> findByCode(String code);

    /** 按访问策略查，id 升序。 */
    List<ModelGroupPO> findByAccessPolicyOrderByIdAsc(String accessPolicy);

    /** 按 id 集合批量查存活模型组。 */
    List<ModelGroupPO> findByIdIn(List<Long> ids);
    ...
    /** 按 id 查存活实体（更新/软删除定位）。 */
    Optional<ModelGroupPO> findById(Long id);
}
