...
import com.nexa.ops.domain.setup.SetupStatus;
...
public record SetupStatusView(
     ...
     */
    public static SetupStatusView from(SetupStatus status) {
        if (status.initialized()) {
            // 已初始化：仅 status=true，其余 null → NON_NULL 不序列化。
            return new SetupStatusView(true, null, null);
        }
        return new SetupStatusView(false, status.rootExists(), status.databaseType());
    }
}
